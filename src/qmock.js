...
(function ( container, undefined ) {
   ...
   * 
   * Factory method for instantiating a new QMock instance with separate state and copies 
   *  of private/public members
   *  
   *  This is potentially useful for running two versions side-by-side with custom 
   *  comparison methods, or modifiying any of the public members.
   *  
   *  #### Example
  ...
  function createQMock() {
    /**
     * == Config ==
     * Configuration settings for QMock - can be modified during runtime. Exposed as 
     *  <code>QMock.config</code>.
     **/

    /** section: Config
     * Config
     *  _Note_: This is not a global object, but simply a separate documentation section 
     *  for QMock config options
     **/
    var config = {
      /**
       * Config.failFast -> Boolean
       *  
       *  Allows a developer to specify whether errors built up during a verify phase should 
       *  be thrown or suppressed. If they are thrown then it is up to the collaborator test 
       *  runner code to catch them and handle. 
       *  
       *  This can result in scenarios where expected 'good' exercise phase fails, an 
       *  exception is thrown, and the testrunner stops due an unhandled exception.
       *  
       *  An alternative is to adopt a 'fail slow' appraoch, by setting 
       *  <code>QMock.config.failFast</code> to <code>false</code>, and use the 
       *  <code>Mock.exceptions()</code> method to inspect the error state.
       *  
       *  #### Example
       *  <pre><code>QMock.failFast = false;</code></pre>
       **/
      failFast: true,
      /**
       ...
       *  <code>Boolean</code> value, and be set before any tests are run, or mock 
       *  instantiated.
       *
       *  #### Example
     ...
     *  - method (Mock | Member): Mock object to which the presentation is / would 
     *  be passed
     ...
     *  - property (String) _optional_: Optional key used to lookup associated data 
     *  held on an expectation object
     *
     ...
     *  during an exercise phase. The method can be used as standalone to test the Stub 
     *  interface.
     *
     *  If no property parameter passed then a Boolean value is returned depending on 
     *  match success.
     *
     *  #### Example
     ...
     *
     *  Necessary as QMock supports the setting of expected parameters (via accepts 
     *  property), or callback arguments (via 'data') without having to be serialised 
     *  as arrays where there is only one parameter.
     *
     *  e.g.
     ...
     *
     *  Factory for creating a mock stub function - acts as a mutator and operates on a 
     *  specific mock object state.
     *  Instance state is mutated when a stub is invoked as part of a 'system under test' 
     *  (SUT) exercise phase.
     *
     *  _Returns_: Closure-bound mock stub function. Function object also has static 
     *  accessor (<code>_getState</code>) which returns the internal state of the 
     *  bound mocked method instance as an object. This can be useful for debugging 
     *  purposes.
     **/
    function createStub ( mock ) {
       ...
       * 
       *  Utility method for retrieving the internal state of a mock object for debugging
       **/
     ...
     *  - receiver (Object) _optional_: Receiver object to associate 'hold' returned Member.
     *
     ...
     *  - definition (Hash): Hash of Mock expectations mapped to Mock object API.
     *
     *  Function which interprets a JSON mapping of a mock object interface 
    ...
    function createMock ( mock, definition ) {
      ...
      if ( typeof mock.expects === "undefined" || definition != null ) {
        throw new Error("createMock() requires a defintiion map {}");
      }

      // protected keys on mock object interface
      var blacklisted = /^(?:_called|_min|_max)$/,
          entry;
              ...
              for (var expectation in memberConfig) {
                if ( hasOwnProperty.call( memberConfig, expectation ) ) {
                  ...
                  if ( (member[expectation] !== undefined)
                    ...
                    // throwMockException("Identifier for method on new Mock instance", 
                    // "Mock." + member["name"] + "[" + expectation + "]", 
                    //"InvalidMockInstanceMethodException", member["name"] + '.' + expectation);
                  }
     ...
     *
     *  If the presentation made to the mock object interface contains a function object, 
     *  then the presentation is tested for a matching 'data' expectation on the mocked 
     *  method instance.
     *
     *  If a match is found then the canned data parameters are passed to what is assumed 
     *  to be callback and it is invoked.
     *
     ...
     *
     *  If match found then lookup is made for a corresponding 'returns' property. 
     *  If not found then catch-all 'return' value is returned, which defaults to 
     *  <code>undefined</code> (as per spec).
     **/
    ...
    function verifyInvocations ( mock ) {
      return ( mock._minCalls == null )
        ...
        : (
          ...
          || ( mock._minCalls <= mock._called ) && ( mock._maxCalls >= mock._called )
          // at least n calls
          || ( mock._minCalls < mock._called ) && ( mock._maxCalls === Infinity ) );
    }
     ...
     * - method (Mock | Stub): mock object to test
     *
     *  Evaluates if number of parameters passed to mock object falls below / exceeeds 
     *  expectations
     **/
     ...
     *
     *  Evaluate a single presentation against all mock object interface expectations. 
     *  Single match equals true.
     **/
     ...
     *  - raise (Function) _optional_: Function to handle false comparison results
     *
     *  Evaluate *all* presentations made to mock object interface against all mock 
     *  interface expectations. Each presentation must match an expectation. If no match 
     *  and optional error handler passed then error raised.
     **/
     ...
     *  - raise (Function) _optional_: Function to handle false comparison results
     *
     *  Verifies the receiver object (the parent mock object) first, then individual 
     *  members. Only passes if whole object tree passes, else throws exception (fail fast).
     **/
     ...
     *
     *  Utility method to reset the state of any mock object to before any interaction 
     *  was recorded.
     **/
    ...
    /**
      ...
      *
      * Resets internal state of the receiver mock object to before any interaction 
      *  occurred, and any child mock objects associated with it.
      **/
     ...
     *  - min (Number) _optional_: Miniumum number of times mocked method should be called. 
     *  If max parameter not passed then number becomes a 'strict' invocation expectation 
     *  (even zero).
     *  - max (Number) _optional_: Maximum number of times mocked method should be called. 
     *  If want 'at least _n_' then just pass Infinity.
     *
    ...
    Member.prototype = {
      ...
      /**
       * Mock#name( descriptor ) -> Mock
       *  - identifier (String): ID of the mock object
       *
       *  Identifier is used to create meaningful error messages. By default it is 
       *  <code>"anonymous"</code>, or the method name (via <code>Mock.method</code>).
       *
       *  #### Example
       ...
       *
       *  When <code>method()</code> is called on a receiver object it is augmented with 
       *  a new method bound to the identifier <code>name</code>.
       *
       ...
       *
       *  Where the <code>returns</code> & <code>data</code> properties are _optional_. 
       *  For more info on these properties see <code>Mock.returns</code> and 
       *  <code>Mock.data</code> resepctively.
       *
       *  Method can be overloaded with as many expectations as required. During 
       *  verification each actual <code>presentation</code> made to a mock object 
       *  interface is tested against all expectations for a match.
       *
       *  #### Example
       *
       ...
       *
       *  Method is used to set a single expected parameter list of _n_ length for a 
       *  mock object. During verification this is tested against the actual 
       *  <code>presentation</code> made to a mock object interface.
       *
       *  When called multiple times on a mock object the last expectation takes precedent.
       *  If multiple expectations are required see the <code>Mock.interface</code> method.
       *
       *  #### Example
       *
       ...
       *
       *  _Note_: This return value simply overrides the default of <code>undefined</code>.
       *  It does not correlate with any particular parameter expectations. See 
       *  <code>Mock.interface</code> to bind return values to an expected parameter list 
       *  (a <code>presentation</code>).
       *
       *  By default mock objects return <code>undefined</code> as per ECMA specification.
       *
       *  #### Example
       ...
       *
       *  A mock object interface can accept parameters, which can be either required or 
       *  optional. By default the number of required parameters is set by 
       *  <code>Mock.accepts</code> to the length of the expected parameter list, but 
       *  <code>Mock.required</code> can be used to override this in the case where some 
       *  parameters are optional.
       *
       *  It is used in conjunction with <code>Mock.overload()</code> to set parameter 
       *  expectations on a mock object interface.
       *
       *  #### Example
       *
       ...
       *
       *  Verification of presentations to interface will allow 0, 1, or 2 parameters to be 
       *  passed (though will still test type/value)
       *
       *  _Note_: In the above example, if method overloading has been set to false then 
       *  the verify phase would only allow presentations of 0 parameters to the mock 
       *  object interface.
       **/
       ...
       *
       *  This flag determines whether the required number of parameters is a strict 
       *  expectation or not.
       *
       *    * If overloading is set to <code>true</code> (default setting) then the 
       *  verify phase will pass presentations to the mock object interface with _at least_ 
       *  the required number of parameters.
       *    * If overloading is set to <code>false</code> then the verify phase will only 
       *  pass presentations to the mock object interface of _exactly_ the required number 
       *  of parameters.
       *
       *  #### Example
      ...
      /**
       * Mock#data() -> Mock
       *  - data (Array| Object): Array of values, or single Object, which is passed as 
       *  arguments to a callback function set on a mock object parameter list expectation
       *
       *  Method allows developer to declare stubbed data (e.g. a web service response 
       *  or DOM elements) to pass to callback functions defined passed to the mock object 
       *  interface in an exercise phase.
       *  
       *  This is most commonly done to test asynchronous operations or event callbacks. 
       *  See the QMock wiki for more use cases and patterns.
       *
       *  #### Example
       ...
       *
       *  When <code>method()</code> is called on a receiver object it is augmented with a 
       *  new property bound to the identifier <code>prop</code> with the value 
       *  <code>val</code>.
       *
       ...
       * Mock#chain() -> Mock
       *
       *  Tells the mocked method to return the receiver object it is bound to enable 
       *  cascading (chained) invocations during the exercise phase.
       *
       *  #### Example
       ...
       *  </code></pre>
       *
       *  _Note_: Can be overwritten if <code>Mock.returns</code> is used after in Mock 
       *  declaration (or better, in conjunction to override it with 
       *  <code>Mock.interface</code> for specific use cases)
       **/
      "chain": function () {
       ...
       *
       *  Tests actual mock interaction with expected interaction on mock object and any 
       *  associated mock objects. Happens in the following order:
       *
       *    * Verify number of invocations
       *    * Verify number of parameters passed to interface
       *    * Verify all sets of parameters passed to interface
       *
       *  Method will bail as any point the verification is false (fail fast), and return 
       *  <code>false</code>.
       *
       ...
       *  - num (Number): Number of times mock object should _at least_ be invoked.
       *
       *  Utility method (well, syntactic sugar) for setting up invocation expectations of 
       *  'at least _n_ invocations'.
       *
       ...
       *  - num (Number):
       *
       *  Utility method (well, syntactic sugar) for setting up invocation expectations 
       *  of 'no more than _n_ invocations'.
       *
       *  Recommended to use <code>Mock.expects</code> or <code>Mock.calls</code> instead.
       **/
       ...
       *  - min (Number): Miniumum number of times mock object should be invoked. If 
       *  max parameter not passed then number becomes a 'strict' invocation expectation 
       *  (even zero). Default is <code>null</code>.
       *  - max (Number) _optional_: Maximum number of times mocked method should be called. 
       *  If want 'at least _N_' then just pass Infinity. Default is <code>null</code>
       *
       *  This method mainly used to set invocation expectations on constructors / receiver 
       *  objects, as conventional usage for mock methods is to pass parameters to 
       *  <code>Mock.expects</code>.
       *
       *  However, <code>Mock.calls</code> can be called on an instance of <code>Mock</code>.
       *
       *  #### Examples
       *
       ...
       * Mock#end() -> Mock
       *
       *  Utility function to end a mock method expectation declaration, or retrieve the 
       *  receiver object a mock object is bound to.
       *
       *  This can be though of analogous to jQuery's <code>end</code> method which is 
       *  used on 'wrapped sets' to restore a collection to it's original state 
       *  post-destructive operations (like <code>filter</code>).
       *
       *  #### Example
     ...
     *  - definition (Hash): Hash of Mock expectations mapped to Mock object API.
     *
     *  Constructor for mock receiver, methods and properties. The return object is a 
     *  function that can act as a namespace object (aka a 'receiver'), or as a 
     *  mocked function / constructor with expectations, or both (e.g. jQuery $).
     *
     *  #### Example
     *  <pre><code>// Via API
     *  var ninja = new Mock();
    ...
    function Receiver ( definition ) {
       ...
       *  (even zero). Default is <code>null</code>.
       * - max (Number) _optional_: Maximum number of times mocked method should be called. 
       *  If want 'at least _n_' then just pass <code>Infinity</code>. Default is 
       *  <code>null</code>
       *
       *  Factory for creating new mock objects (methods / properties) on the mock receiver 
       *  object.
       *
       ...
       *  <code>Mock.verify()</code> returns <code>false</code> in 'fail slow' test 
       *  runner setups (see Config section).
       **/
