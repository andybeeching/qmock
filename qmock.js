	...
	*/
	...
	Mock = (function createMockConstructor () {
		...
		return function MockConstructor () {
		                ...
		                return function updateMethodState () {
		                    ...
		                    return (function getReturnValue(presentation) {
										...
										for (var i = 0, len = method.expectedArgs.length; i < len; i++) {
											if ( 
												assertArray (
													method.expectedArgs[i]["accepts"], // expected interface presentation
													presentation, // actual
					...
					if ( true === isMethod ) {
						...
						for (var expectation in memberConfig) {
							...
							if ( (member[expectation] !== undefined) 
								...
								// Disco.
								member[expectation][
									( (expectation === "interface" || expectation === "accepts") && memberConfig[expectation].constructor === Array)
										? "apply"
										: "call"
			...
			expose( assertArray, MockConstructor, "_assertArray" );
			expose( assertObject, MockConstructor, "_assertObject" );
			expose( assertCollection, MockConstructor, "_assertObject" );
			expose( buildException, MockConstructor, "_buildException" );
